...
- `CRT_SAVE_INTERVAL_S` (seconds, default `30`) — how often the daemon persists state to disk via the `periodic_save_loop` background task in `crt/daemon.py`. State is no longer saved only on graceful shutdown, so a crash/OOM/reboot now loses at most one interval of cursor/loop_mode/playback_position changes. The shutdown path keeps its own final flush.

**Persistence:** Queue state (items, history, playback position) saved to `~/.local/share/crt-player/state.json` (configurable via `CRT_STATE_FILE`). `periodic_save_loop` persists every `CRT_SAVE_INTERVAL_S` (default 30s) **and** on graceful exit (issue #4 — a hard kill no longer loses the cursor). `save_state(path, playback_position=X)` writes the live position onto the cursor item before serializing so a restart resumes from the pause point. On reload, mid-processing items reset to `"queued"`; playing items become `"ready"` if encoded file exists (skips download+encode).

**Control acks:** `POST /control/{next,prev,toggle,stop,play,seek/*}` return a structured ack `{"ok": true, "did_action": bool, "reason": str|null, ...}` (issue #6). `PlayerCore` methods return an `ActionResult(did_action, reason)`; `api.py`'s `_ack()` translates it. `reason` ∈ `{no_items, no_playable_item, cursor_unchanged, no_chromecast_session, null}`. A no-op (e.g. nothing ready to cast, or a seek with no live session) is now `did_action=false` instead of an indistinguishable 200 OK. **Follow-up (separate repo):** the `crt-flipper-bridge` should log `did_action=false` at WARN — not yet done.
...
**Tailnet access from Mac:** `CRT_DAEMON_URL=http://chet.<tailnet>.ts.net:8765 crt-tui`. Daemon listens on `0.0.0.0:8765` but has no UFW LAN rule since 2026-05-11 — external access only via Tailscale.
...
**Re-OAuth shortcut:** scope-only re-deploy after re-running `crt-bootstrap` on Mac doesn't need full `lodge crt-player install`: `scp ~/.local/share/crt-player/oauth_token.json flocca@chet.tailc1c72f.ts.net:/opt/lodge/crt-player/secrets/ && ssh flocca@chet.tailc1c72f.ts.net "chmod 600 /opt/lodge/crt-player/secrets/oauth_token.json && docker restart lodge-crt-player"`. (Daemon moved to Chet in split-host migration 2026-05-22.)
